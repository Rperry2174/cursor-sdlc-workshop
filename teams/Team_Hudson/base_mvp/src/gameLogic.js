 ...
 *
 * This version supports different difficulty levels by allowing
 * scroll speed, spawn interval, and max iceberg count to be overridden.
 */
 ...
 *  - maxIcebergs
 */
export function createInitialState(width, height, overrides = {}) {
  ...
  return {
    ...
    riverRight,
    riverWidth,
    scrollSpeed: overrides.scrollSpeed ?? SCROLL_SPEED,
    spawnIntervalMs: overrides.spawnIntervalMs ?? SPAWN_INTERVAL_MS,
    milesPerPixel: MILES_PER_PIXEL,
    milesToWin: MILES_TO_WIN,
    maxIcebergs: overrides.maxIcebergs ?? DEFAULT_MAX_ICEBERGS,
  };
}
...
export function maybeSpawnIceberg(state, now) {
  ...
  if (typeof state.maxIcebergs === 'number' && state.icebergs.length >= state.maxIcebergs) return;
