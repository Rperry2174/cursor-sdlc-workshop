 ...
 *
 * Accepts a `difficultyConfig` prop so spawn rate, scroll
 * speed, and max iceberg count can change with difficulty.
 */
...
export default function GameCanvas({ onWin, onLose, difficultyConfig }) {
  ...
  const initState = useCallback(() => {
    if (!stateRef.current) {
      stateRef.current = createInitialState(CANVAS_WIDTH, CANVAS_HEIGHT, difficultyConfig || {});
    } else {
      const fresh = createInitialState(CANVAS_WIDTH, CANVAS_HEIGHT, difficultyConfig || {});
